...
class ImageViewer(QWidget):
    ...
    def print_screen(self):
        ...
        if self.show_track.isChecked():
            ...
            for line in lines:
                ...
                color = get_color_by_id(label_id)
                ...
                text_rect = QRectF(x1, y2, font_size * len(str(label_id)), font_size * 1.5)
                painter.fillRect(text_rect, color)
                painter.setPen(QPen(Qt.black, 1))
                painter.drawText(text_rect, Qt.AlignCenter, str(label_id))
                
                if self.highlighted_track_id == label_id:
    ...
    def update_tracking_info(self):
        # 读取summary.json文件
        summary_path = os.path.join(self.labels_dir, 'summary.json')
        if os.path.exists(summary_path):
            with open(summary_path, 'r') as f:
                summary_data = json.load(f)
                summary_data = summary_data["objects"]
        else:
            summary_data = []
        ...
        # 遍历跟踪信息并添加到跟踪信息显示区域
        for track_info in summary_data:
            # 创建ID条目
...
if __name__ == '__main__':
    app = QApplication([])
    images_dir = r"C:\Users\qj00241.7HORSE\Downloads\MLT\tl_imgs\dishui_lake_1"
    labels_dir = r"C:\Users\qj00241.7HORSE\Downloads\MLT\tl_labels\dishui_lake_1"
    preds_dir = r"C:\Users\qj00241.7HORSE\Downloads\MLT\tl_preds\dishui_lake_1"
    tracks_dir = r"C:\Users\qj00241.7HORSE\Downloads\MLT\tl_tracks\dishui_lake_1"
    
    viewer = ImageViewer(images_dir, tracks_dir) # labels_dir
    viewer.show()
    app.exec_()
